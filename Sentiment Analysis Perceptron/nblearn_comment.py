...
class MultinomailNbTrain():
    ...
    def count_feats(self):  # STEP 2
        ...
        Store the result in self.class_vocab_size
        class_type - the type of class you want to count features for either belief or sentiment. It is same'''
    ...
    def plus_one_smooth_and_cond_prob(self):  # STEP 3
        ...
        # STEP 3: For each class/label/feature_set, divide by: (class/label prior + class_vocab_size)
        for c in self.class_feat_counts.keys():
            labels = list(self.class_feat_counts[c].keys())
            for l in labels:
                for key in self.class_feat_counts[c][l].keys():
