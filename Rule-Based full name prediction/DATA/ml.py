...
class nameObj():
    ...
    def firstname_token_perc(self, name):
...
def predict(name_one, name_two, gender_dicts, surname_dict):
    ...
    name1fn = name_one.firstname_token_perc(name_one.full_name[-1])
    name1ln = name_one.lastname_token_perc(name_one.full_name[-1])

    name2_2_fn = name_two.firstname_token_perc(name_two.full_name[-2])
    name2_2_ln = name_two.lastname_token_perc(name_two.full_name[-2])

    name2_1_ln = name_two.lastname_token_perc(name_two.full_name[-1])
    ...
    if len(name_one.full_name) == 1:
        ...
        elif name_two.firstname_token_perc(name_two.full_name[-2]) >= name_two.lastname_token_perc(name_two.full_name[-2]):
            result = add_last_token(name_one.name, name_two.name)
            return result
        else:
            result = add_last_two_token(name_one.name, name_two.name)
    ...
    if len(name_one.name) == 2:
        ...
        b1 = name_two.lastname_token_perc(name_two.full_name[-2])   
        ...
        if len(name_two.name) == 4:
            ...
            if a > 0.001 and c < 0.001:
                result = add_nothing(name_one.name)
            elif c > 0.00 and b == 0.0:
                result = add_last_token(name_one.name, name_two.name)
            else:    
                result = add_last_two_token(name_one.name, name_two.name)
            ...
            if (a == 0.0 or a > 0.003) and (c == 0.0 or c < 0.001):
                result = add_nothing(name_one.name)
                #return result
            if b > 0.03:
                result = add_last_two_token(name_one.name, name_two.name)   
...
printPartAccuracy(1, 2, False) #1,2=96 1,3=94 1,4=95 == 95
printPartAccuracy(2, 3, True) #2,2=82 2-3=75 2,4=73.80 == 77
...
#PREDICT
data_key_file = "data-key-ml.csv"
key_file = 'dev-key.csv'
key_lines = open(key_file, 'r')
with open(data_key_file, 'w', newline='') as file:
    writer = csv.writer(file)
    for line,line1 in zip(lines, key_lines):
        line = line.strip()
        name_list_str = line.split(' AND ')
        ...
        names = line1.split(',')
        label = len(names[1].split())
        data_final = [name1fn, name1ln, name2_2_fn, name2_2_ln, name2_1_ln, label]
        writer.writerow(data_final)
"""
df = pd.read_csv("data-key-ml.csv", header=None)
header_list = ['name1fn', 'name1ln', 'name2_2_fn', 'name2_2_ln', 'name2_1_ln', 'target'] 
df.to_csv("data-key-ml.csv", header=header_list, index=False)
"""
